...
In case you need to extract the results from proportion tests in a separate
dataframe, `ggpiestats`'s `output` argument can be used:

```{r proptest_df}
set.seed(123)

ggpiestats(mtcars, am, cyl, output = "proptest", results.subtitle = FALSE)
```
...
`ggstatsplot::combine_plots`. It applies `ggpiestats` across all **levels** of a
