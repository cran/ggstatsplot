...
`cpglm`, `cpglmm`, `crch`, `crq`, `DirichReg`, `drc`, `emmGrid`, `epi.2by2`,
`ergm`, `feis`, `felm`, `fitdistr`, `fixest`, `flexsurvreg`, `gam`, `Gam`,
`gamlss`, `garch`, `geeglm`, `glmc`, `glmerMod`, `glmmTMB`, `gls`, `glht`,
`glm`, `glmm`, `glmmadmb`, `glmmPQL`, `glmRob`, `glmrob`, `glmx`, `gmm`,
...
`svyolr`, `svyglm`, `tobit`, `truncreg`, `vgam`, `vglm`, `wbgee`, `wblm`,
`zeroinfl`, etc.
...
# plot
ggstatsplot::combine_plots(
  ggstatsplot::ggcoefstats(m1_afex, title = "example-1"),
  ggstatsplot::ggcoefstats(m2_afex, title = "example-2"),
...
```{r censReg, fig.height = 5, fig.width = 5, eval=FALSE}
# setup
set.seed(123)
library(censReg)
data("Affairs", package = "AER")
...
# combining plots
ggstatsplot::combine_plots(
  ggstatsplot::ggcoefstats(mod_fx1),
  ggstatsplot::ggcoefstats(mod_fx2),
  ggstatsplot::ggcoefstats(mod_fx3),
  ggstatsplot::ggcoefstats(mod_fx4),
  labels = "auto",
  title.text = "fast and user-friendly fixed-effects estimations"
)
```
