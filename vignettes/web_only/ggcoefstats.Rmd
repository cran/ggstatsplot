...
```{r svyolr, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 8}
# setup
set.seed(123)
library(survey)
data(api)
...
```{r robust, eval = FALSE, warning = FALSE, message = FALSE, fig.height = 12, fig.width = 8}
ggstatsplot::combine_plots(
  # plot 1: glmRob
  ggstatsplot::ggcoefstats(
    x = robust::glmRob(
...
set.seed(123)
mod <-
  speedglm::speedlm(formula = mpg ~ wt + qsec,
                    data = mtcars,
                    fitted = TRUE)
...
# model
g <- gam::gam(formula = mpg ~ s(hp, 4) + am + qsec, 
              data = mtcars)

# plot
ggstatsplot::ggcoefstats(x = g, 
...
# base models
b <-
  stats::lm(formula = job_seek ~ treat + econ_hard + sex + age, 
            data = jobs)
c <-
...
```{r orcutt, eval = FALSE, warning = FALSE, message = FALSE, fig.height = 4, fig.width = 4}
# model
library(orcutt)
set.seed(123)
...
# plot
ggstatsplot::ggcoefstats(x = fit1,
                         exclude.intercept = FALSE,
...
# plot
ggstatsplot::ggcoefstats(x = df_min,
...
```{r meta2, warning = FALSE, message = FALSE, fig.height = 7, fig.width = 6}
...
# plot
ggstatsplot::ggcoefstats(x = df_full,
                         meta.analytic.effect = TRUE, 
                         statistic = "t",
