...
`aareg`, `anova`, `aov`, `aovlist`, `Arima`, `bayesx`, `bayesGARCH`, `BBmm`,
`BBreg`, `bcplm`, `bglmerMod`, `bife`, `bigglm`, `biglm`, `blavaan`, `bmlm`,
`blmerMod`, `bracl`, `brglm2`, `brmsfit`, `brmultinom`, `btergm`, `cch`, `cgam`,
`cgamm`, `cglm`, `clm`, `clm2`, `clmm`, `clmm2`, `coeftest`, `complmrob`,
`confusionMatrix`, `coxme`, `coxph`, `cpglm`, `cpglmm`, `crch`, `DirichReg`,
`drc`, `emmGrid`, `epi.2by2`, `ergm`, `feis`, `felm`, `fitdistr`, `flexsurvreg`,
`glmc`, `glmerMod`, `glmmTMB`, `gls`, `gam`, `Gam`, `gamlss`, `garch`, `glm`,
`glmmadmb`, `glmmPQL`, `glmRob`, `glmrob`, `glmx`, `gmm`, `hurdle`, `ivreg`,
`iv_robust`, `lavaan`, `lm`, `lm.beta`, `lmerMod`, `lmerModLmerTest`, `lmodel2`,
`lmRob`, `lmrob`, `LORgee`, `lrm`, `mcmc`, `mcmc.list`, `MCMCglmm`, `mclogit`,
`mmclogit`, `mediate`, `mixor`, `mjoint`, `mle2`, `mlm`, `multinom`, `negbin`,
`nlmerMod`, `nlrq`, `nlreg`, `nls`, `orcutt`, `plm`, `polr`, `ridgelm`, `rjags`,
`rlm`, `rlmerMod`, `robmixglm`, `rq`, `rqss`, `slm`, `speedglm`, `speedlm`,
...
# model
mod <- stats::aov(formula = rating ~ mpaa * genre, data = movies_10)
...
ggstatsplot::combine_plots(
  # model 1
  ggstatsplot::ggcoefstats(
    x = stats::aov(formula = rating ~ mpaa, data = movies_10),
    title = "1. Only MPAA ratings"
  ),
  # model 2
  ggstatsplot::ggcoefstats(
    x = stats::aov(formula = rating ~ genre, data = movies_10),
    title = "2. Only genre"
  ),
  # model 3
  ggstatsplot::ggcoefstats(
    x = stats::aov(formula = rating ~ mpaa + genre, data = movies_10),
...
```{r anova_car, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 6}
# setup
set.seed(123)
library(car)
library(ggstatsplot)
...
```{r anova_ez, warning = FALSE, message = FALSE, fig.height = 4, fig.width = 4}
set.seed(123)
library(ez)
data(ANT)
...
```{r lmerModLmerTest, warning = FALSE, message = FALSE, width = 6, height = 12}
# setup
set.seed(123)
library(lmerTest)
...
```{r glm2, warning = FALSE, message = FALSE, fig.height = 5, fig.width = 5}
# setup
library(glm2)
y <- c(1, 1, 1, 0)
...
```{r lrm, warning = FALSE, message = FALSE, fig.height = 12, fig.width = 7}
# setup
library(rms)
set.seed(123)
...
```{r BBmm, warning = FALSE, message = FALSE, fig.height = 5, fig.width = 6}
# setup
library(PROreg)
set.seed(123)
...
```{r robmixglm, warning = FALSE, message = FALSE, fig.height = 4, fig.width = 4}
# setup
set.seed(123)
library(robmixglm)
library(MASS)
...
```{r clm2, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 6}
# for reproducibility
set.seed(123)
library(ordinal)
library(MASS)
...
```{r brmultinom, warning = FALSE, message = FALSE, fig.height = 12, fig.width = 7}
# setup
set.seed(123)
library(MASS)
library(brglm2)
...
```{r bracl, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 7}
# setup
set.seed(123)
library(brglm2)
data("stemcell")
...
ggstatsplot::combine_plots(
  ...
  title.text = "Robust variants of `lmRob` and `glmRob` (from`robust` package)",
  nrow = 2
)
```
...
ggstatsplot::combine_plots(
  ...
  # arguments relevant for `combine_plots` function
  title.text = "Robust variants of `lmRob` and `glmRob` (from`robustbase` package)",
  nrow = 2
)
```
...
```{r Gam, warning = FALSE, message = FALSE, fig.height = 5, fig.width = 5, eval=FALSE}
# setup
set.seed(123)
library(gam)
...
ggstatsplot::ggcoefstats(
  x = dax.garch,
  title = "generalized autoregressive conditional heteroscedastic (GARCH)"
)
```

## Bayesian Estimation of the GARCH(1,1) Model with Student-t Innovations (`bayesGARCH`)

```{r bayesGARCH, warning = FALSE, message = FALSE, fig.height = 4, fig.width = 4}
# setup
set.seed(123)
library(bayesGARCH)
...
mod <- bayesGARCH(y, control = list(n.chain = 4, l.chain = 200))
...
```{r wblm, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 7, eval=FALSE}
# setup
set.seed(123)
library(panelr)
...
```{r wbgee, warning = FALSE, message = FALSE, fig.height = 8, fig.width = 6, eval=FALSE}
# setup
set.seed(123)
library(panelr)
data("WageData")
...
```{r bayesx, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 6}
# setup
set.seed(111)
library(R2BayesX)
