...
`blmerMod`, `brmsfit`, `btergm`, `cch`, `clm`, `clmm`, `confusionMatrix`,
`coxph`, `drc`, `emmGrid`, `epi.2by2`, `ergm`, `felm`, `fitdistr`, `glmerMod`,
`glmmTMB`, `gls`, `gam`, `Gam`, `gamlss`, `garch`, `glm`, `glmmadmb`, `glmmPQL`,
`glmmTMB`, `glmRob`, `glmrob`, `gmm`, `ivreg`, `lm`, `lm.beta`, `lmerMod`,
...
`speedglm`, `speedlm`, `stanreg`, `survreg`, `svyglm`, `svyolr`, `svyglm`,
`tobit`, `wblm`, etc.
...
```{r negbin, warning = FALSE, message = FALSE, fig.height = 7, fig.width = 6}
# setup
library(MASS)
library(lme4)
set.seed(101)
...
```{r bglmerMod, warning = FALSE, message = FALSE, fig.height = 5, fig.width = 5}
# for reproducibility
set.seed(123)
library(blme)
...
ggstatsplot::combine_plots(
  ...
  # plot 2: lmrob
  ggstatsplot::ggcoefstats(
    x = m1 <- robustbase::lmrob(formula = Y ~ ., data = coleman),
    title = "robust linear model"
  ),
  # arguments relevant for `combine_plots` function
  labels = c("(i)", "(ii)"),
...
```{r mmclogit, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 6}
# setup
set.seed(123)
library(mclogit)
data(electors)
...
## panel regression models fit via multilevel modeling (`wblm`/`panelr`)
