 ...
 * A vector of values such as `mean.label.size = c(3, 4, 5)` in which case it
   ...
   `mean.label.size = c(3, 4)` will fail if we're trying to make three plots.

 * A list; either named `data = year_list` or created as you go
   `outlier.label.color = list("#56B4E9", "#009E73", "#F0E442")`. Any list will
...
So let's keep building `.l`. First is `data = year_list`, the `x` & `y` axes are
constant in all three plots so we pass the variable name as a string `x = "continent"`. 
...
plot_list <-
  purrr::pmap(
    .l = list(
      ...
      corr.method = list("pearson", "np", "robust", "robust"),
...
plot_list <-
  purrr::pmap(
    .l = list(
      ...
      test.value.line.args = list(
        list(color = "#E69F00", size = 1),
        list(color = "#56B4E9", size = 0.8),
        list(color = "#009E73", size = 1.5),
        list(color = "blue", size = 0.4)
      ),
      centrality.parameter = list("median", "mean", "mean", "none"),
      ggtheme = list(
...
```{r purrr_ggpiestats, warning = FALSE, message = FALSE, fig.height = 22, fig.width = 9}
# for reproducibility
set.seed(123)
...
```{r purrr_ggbarstats, warning = FALSE, message = FALSE, fig.height = 24, fig.width = 6}
# for reproducibility
set.seed(123)
...
```{r purrr_columns, warning = FALSE, message = FALSE, fig.width=12}
# setup
set.seed(123)
library(ggstatsplot)
library(patchwork)
