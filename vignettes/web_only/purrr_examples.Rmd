 ...
 * A vector of values such as `mean.ci = c(TRUE, FALSE, TRUE)` in which case it
   ...
   `mean.ci = c(TRUE, FALSE)` will fail if we're trying to make three plots.
...
a 3 item named `list` called `data_list` using `dplyr::filter` and `split`.
